...
def run_accounting(input_path: Path, out_dir: Path, fmt: str, limit: int | None,
    ...
    def _progress(idx, total, appno, row):
        if row["asset_status"] == "unsupported":
            print(f"  [{idx}/{total}] {appno}  unsupported (권리구분 {row['right_code']})")
        elif row["asset_status"] == "검토필요" and "중복" in (row.get("basis") or ""):
            print(f"  [{idx}/{total}] {appno}  검토필요 (중복)")   # SHOULD-2: 기존 마커 보존
        else:
            print(f"  [{idx}/{total}] {appno}  {row['asset_status']:9s} "
                  f"계정={row['account'] or '-':16s} ({row['legal_state']})")

    rows = engine.classify_entries(entries, access_key, source=source, delay=delay,
